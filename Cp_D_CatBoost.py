...
# 选择要预测的目标变量
target_var = 'Cp_D'  # 可以改为 'Cp_B', 'Cp_C', 'Cp_D'
...
# 创建完整管道工厂函数
# ==============================================
def create_full_pipeline(catboost_params=None):
    """
    创建完整管道，方便重复使用 - CatBoost版本
    """
...
# 模型构建（使用负的相对RMSE）
# ==============================================
def catboost_cv_score(n_estimators, max_depth, learning_rate, subsample,
                    random_strength, l2_leaf_reg):
    """
    CatBoost贝叶斯优化目标函数 - 使用负的相对RMSE
    """
    try:
        # 参数转换
        ...
        # 添加正则化惩罚
        penalty = 1.0
        if max_depth > 10:  # 对过深的树进行惩罚
            penalty *= 0.95
        if learning_rate < 0.01:  # 对过小的学习率进行惩罚
            penalty *= 0.95

        return -avg_relative_rmse * penalty
...
# ==============================================
param_bounds = {
     'n_estimators': (185, 210),
    'max_depth': (5, 7),
    'learning_rate': (0.25, 0.32),
    'subsample': (0.8, 0.9),
...
try:
    ...
    optimizer = BayesianOptimization(
        f=catboost_cv_score,  # 使用XGBoost目标函数
...
# 创建输出目录
output_path = f"E:\\胡琰琰\\毕业论文内容\\G\\运行数据\\拟合\\D\\{target_var}_CatBoost_安全版.xlsx"
...
# 1. 创建模型保存目录
model_save_dir = r"E:\\胡琰琰\\毕业论文内容\\G\\运行数据\\拟合\\D\\保存的模型"
os.makedirs(model_save_dir, exist_ok=True)

# 2. 保存完整pipeline（包括预处理和模型）
model_filename = f"{target_var}_CatBoost_模型_安全版.pkl"
