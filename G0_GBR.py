...
# 选择要预测的目标变量
target_var = 'G0'  # 可以改为 'Cp_B', 'Cp_C', 'Cp_D'
...
# ==============================================
param_bounds = {
    'n_estimators': (240, 300),  # 最佳268接近原范围上限，小幅扩大区间覆盖邻近最优区域
    'learning_rate': (0.08, 0.11),  # 最佳0.0922在原范围中间，收紧范围提升搜索精准度
    'max_depth': (10, 13),  # 最佳11是原范围下限，缩小区间控制深度（避免过拟合反弹）
    'min_samples_split': (6, 8),  # 最佳7在原范围中间，进一步收紧范围稳定分裂样本数
    'min_samples_leaf': (5, 7),  # 最佳6在原范围中间，聚焦邻近区间增强叶节点样本量
    'subsample': (0.82, 0.86),  # 最佳0.8273接近原范围下限，收紧范围聚焦
    'max_features': (0.96, 1.0)  # 最佳0.9748在原范围中间，聚焦高位区间
}
...
# 创建输出目录
output_path = f"E:\\胡琰琰\\毕业论文内容\\G\\运行数据\\拟合\\G\\{target_var}_GBR_安全版.xlsx"
...
# 1. 创建模型保存目录
model_save_dir = r"E:\\胡琰琰\\毕业论文内容\\G\\运行数据\\拟合\\G\\保存的模型"
